...
def load_existing_results(file_path: Path) -> Dict[str, Dict]:
    ...
    Returns:
        Dict[key, result]: key 由 prompt, chosen, rejected, key_focus_aspects 四个字段生成
    """
...
def _generate_item_key(item: Dict) -> str:
    """
    根据 prompt, chosen, rejected, key_focus_aspects 生成唯一键
    """
    import hashlib
    ...
    key_fields = [
        ...
        item.get("chosen", ""),
        item.get("rejected", ""),
        json.dumps(item.get("key_focus_aspects", []), ensure_ascii=False, sort_keys=True),
    ]
...
def process_judgement(output: str) -> Tuple[Optional[int], str]:
    ...
    Returns:
        (score, raw_output)
        score: "A" 表示 A 更好, "B" 表示 B 更好, 0.5 表示平局, None 表示解析失败
    """
    # 提取最后的判决标记
